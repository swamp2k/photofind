  ...
  return (
    <div className={compact ? 'source-path compact' : 'source-path'}>
      <div className="source-folder-row">
        ...
        {navigation ? <button type="button" className="source-folder-link" onClick={() => navigation.showFolder(folder)} title="Show every photo from this exact folder in Library">{sourceFolderLabel(folder)}</button> : <strong>{sourceFolderLabel(folder)}</strong>}
      </div>
